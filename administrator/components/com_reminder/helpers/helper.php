...
class ReminderHelper
{
	...
	*/
	...
	{
		return ReminderHelperMenu::addSubmenu($alias, $view, $layout);
	}
	...
	*/
	public static function enumList($ctrl, $fieldName)
	{
		...
		return ReminderHelperEnum::_($ctrl . '_' . $fieldName);
	}
	...
	*/
	public static function getActions($itemId = 0)
	{
		...
		foreach ($actions as $action)
			$result->set($action, $user->authorise($action, COM_REMINDER));
	...
	*/
	...
	{
		if (is_array($attribs))
			$attribs = ReminderHelperFile::getAttributesFromLegacy($attribs);

		return ReminderHelperFile::getFileUrl($path, $method, $attribs);
	}
	...
	*/
	public static function headerDeclarations()
	{
		...
		$baseSite = 'components/' . COM_REMINDER;
		$baseAdmin = 'administrator/components/' . COM_REMINDER;
		...
		ReminderHelperHtmlValidator::loadLanguageScript();
		ReminderHelperHtmlValidator::attachForm();
		...
		if ($app->isAdmin())
		{
			...
			self::addStyleSheet($baseAdmin, 'css/reminder.css');
			self::addStyleSheet($baseAdmin, 'css/toolbar.css');

		}
		else if ($app->isSite())
		{
			self::addStyleSheet($baseSite, 'css/reminder.css');
	...
	*/
	public static function loadFork($file)
	{
		...
		$file = preg_replace("#com_reminder#", 'com_reminder/fork', $file);
