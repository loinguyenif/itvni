...
defined('REMINDER_UPLOAD_DEFAULT_DIR') or define("REMINDER_UPLOAD_DEFAULT_DIR", 'COM_MEDIAS');
defined('REMINDER_UPLOAD_DEFAULT_RENAME') or define("REMINDER_UPLOAD_DEFAULT_RENAME", '{ALIAS}.{MIMEXT}');
defined('REMINDER_UPLOAD_CHMOD_FOLDER') or define("REMINDER_UPLOAD_CHMOD_FOLDER", 0754);
defined('REMINDER_UPLOAD_CHMOD_FILE') or define("REMINDER_UPLOAD_CHMOD_FILE", 0644);
defined('REMINDER_IMAGES_MAX_WIDTH') or define("REMINDER_IMAGES_MAX_WIDTH", 1000);
defined('REMINDER_IMAGES_MAX_HEIGHT') or define("REMINDER_IMAGES_MAX_HEIGHT", 1000);
defined('REMINDER_IMAGES_ALLOWED_SIZES') or define("REMINDER_IMAGES_ALLOWED_SIZES", '');
...
class ReminderHelperFile
{
	...
	* @var string
	*/
	protected static $extension = 'com_reminder';
		...
		if (static::checkFileExists($dir, $file))
		{
			switch(strtolower($options["overwrite"]))
			{
				...
				case '0':
					...
					throw new \Exception(JText::sprintf( "REMINDER_UPLOAD_EXISTS", $file->filename));
					break;
	...
	public static function getFileUrlFromElement($form, $element, $value)
	{
		...
		if (!$dir)
		{
			...
			if (!in_array($dir, array_keys($directories)))
				$dir = REMINDER_UPLOAD_DEFAULT_DIR;
		...
		$attribs = ReminderHelperFile::getAttributesFromElement($element);

		$url = (($method == 'indirect') && JFactory::getApplication()->isAdmin()?'administrator/':'')
			. ReminderHelperFile::getFileUrl($indirectPath, $method, $attribs);
	...
	*/
	public static function getUploadMaxSize($string = false, $maxSizeCustom = null)
	{
		...
		if ($string)
			$maxSize = JText::sprintf("REMINDER_UPLOAD_MAX", static::bytesToString($maxSize));
		...
		if (JFile::exists($filePath))
		{
			...
			if (in_array($remove, array('trash')))
			{
				...
				if (!JFile::move($filePath, $trashPath .'/'. JFile::getName($filePath)))
					throw new \Exception(JText::_("REMINDER_UPLOAD_TRASH_IMPOSSIBLE"));
			...
			else if (in_array($remove, array('delete')))
			{
				if (!JFile::delete($filePath))
					throw new \Exception(JText::_("REMINDER_UPLOAD_DELETE_IMPOSSIBLE"));
	...
	*/
	...
	{
		$pattern = (isset($options["rename"])?$options["rename"]:REMINDER_UPLOAD_DEFAULT_RENAME);
	...
	*/
	public static function returnFile()
	{
			...
			)
		{
			...
			{
				$w = (int)min($size->width, REMINDER_IMAGES_MAX_WIDTH);
				$h = (int)min($size->height, REMINDER_IMAGES_MAX_HEIGHT);
				...
				if (($size != "0x0") && !in_array($size, explode(",", REMINDER_IMAGES_ALLOWED_SIZES)))
				{
		...
		else if (!JFile::exists($filePath) || in_array($ext, $forbiddenExt))
		{
			...
			$msg = JText::sprintf( "REMINDER_UPLOAD_FILE_NOT_FOUND", $pathNoDir);
			jexit($msg);
		}
	...
	*/
	public static function sanitizeFolder($path)
	{
		...
		@chmod($folder, REMINDER_UPLOAD_CHMOD_FOLDER);
		if(!is_writable($folder))
			throw new \Exception(JText::sprintf( "REMINDER_UPLOAD_READONLY_FOLDER",$path));
	...
	*/
	public static function uploadFile($dir, $uploadFile, $options = array())
	{
		...
		if (!$dir)
			throw new \Exception(JText::_("REMINDER_UPLOAD_FOLDER_NOT_FOUND"));
		...
		if(empty($uploadFile['name']))
			throw new \Exception(JText::_("REMINDER_UPLOAD_PLEASE_BROWSE"));
		...
		{
			throw new \Exception(JText::sprintf( "REMINDER_UPLOAD_REFUSED_EXTENSION",
				$file->extension,
		...
		{
			throw new \Exception(JText::sprintf( "REMINDER_UPLOAD_REFUSED_MIME",
				$file->mime,
				implode(" - ", $extensions))
			);
		}
		...
		{
			throw new \Exception(JText::sprintf( "REMINDER_UPLOAD_REFUSED_SIZE",
		...
		if (preg_match("/\<\?php\s/", $contents))
			throw new \Exception(JText::_( "REMINDER_UPLOAD_CONTAINS_ERRORS"));
	...
	protected static function uploadFileProcess($dirBase, $filePath, $fileTmp)
	{
		...
		if (!move_uploaded_file($fileTmp, $fileDest))
			if(!JFile::upload($fileTmp, $fileDest))
			{
				...
				throw new \Exception(JText::sprintf( "REMINDER_UPLOAD_ERROR_UPLOAD", $file->filename));
			}


		//Protect file against execution
		@chmod($fileDest, REMINDER_UPLOAD_CHMOD_FILE);
