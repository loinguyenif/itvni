...
class ReminderHelperForm
{
	...
	*/
	public static function getElementGroups($element)
	{
		...
		$model = ReminderHelper::componentModel((string)$element['model'], false);
		...
		foreach ($element->children() as $option)
		{
			...
			if ($model)
			{
				...
				$group->model = (strpos($relation->model, '.')?'':'reminder.') . $relation->model;

				$model = ReminderHelper::componentModel($relation->model, false);
			}
			...
			// Label Key
			$group->alias = ReminderClassModelOrm::tableFieldAlias($group->on, false);
	...
	*/
	public static function getModel($field, $item = false)
	{
		...
		return ReminderHelper::componentModel($model, $item);
	}
