...
* @subpackage	Classes
*/
class ReminderClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	* @var string
	*/
	protected static $extension = 'reminder';
	...
	*/
	public function __construct($config = array())
	{
		...
		// Load the ORM system
		$this->orm = ReminderClassModelOrm::getInstance($this);
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("REMINDER_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	public function deletefiles($pks, $fileFields)
	{
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!ReminderHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	public static function fkTable($fieldname)
	{
		...
		switch($fieldname)
		{
			case 'templete_id': return $tbl.$com. 'templetes';
			case 'package_id': return $tbl.$com. 'packages';
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'ReminderModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_REMINDER . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'ReminderModel');
		if ($modelList)
	...
	*/
	public function integrityRestrict($relation, &$pks, $labelKey, $parentLabelKey, $editLink)
	{
		...
		foreach($pks as $id)
		{
			...
			$children = ReminderHelper::getData($childModel, array(
				'filter' => array(
			...
			if (count($children))
			{
				...
				$model = ReminderHelper::componentModel($relation->foreignModelClass, true);
				...
				$msg = sprintf(JText::_('REMINDER_ERROR_IMPOSSIBLE_TO_DELETE_USED_IN'), ($itemName?$itemName:'item'), implode(', ', $childText));
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_REMINDER . '/models/forms');
		JForm::addFieldPath(JPATH_REMINDER . '/models/fields');
	...
	*/
	public function loadRelation($name, &$item, $orm = array())
	{
		...
		$model = ReminderClassModelOrm::getModel($relation->foreignModelClass);
		if (!$model)
			return;
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'reminderModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Reminder"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_REMINDER . '/fork/models/forms';
	...
	*/
	...
	{
		return ReminderClassModelOrm::queryDump($query, $output);
	}
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("REMINDER_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
