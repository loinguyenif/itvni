...
*/
function ReminderCkBuildRoute(&$query)
{
	$view = null;
	$segments = array();
	if(isset($query['view']))
...
*/
function ReminderCkParseRoute($segments)
{
	$vars = array();
	$view = null;
...
*/
function ReminderCkParseSlug($slug, $model, $aliasKey)
{
	$parts = explode( ':', $slug );
	$id = $parts[0];
...
*/
function ReminderCkBuildSlug($id, $model, $aliasKey)
{
	$item = ReminderHelper::getData($model, array(
...
*/
function ReminderCkRouteConfig()
{
	return array(
		'customers' => array(
			array(
